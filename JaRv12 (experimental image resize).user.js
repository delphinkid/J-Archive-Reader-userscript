...
//todo: try incorporating multiple pictures
...
var imgScaled = window.innerWidth-910;
//console.log(winMult);
...
function timeoutA(){
  timer = setTimeout(clueRead, 2000);
}
  ...
  if (isPause){
    setTimeout(answerRead, 3000);
  	...
  	try{
        ...
        var rawHTML = reHTML.exec(toggler);
      ...
      var textHeight = clue2.getElementsByClassName("clue_text")[0].clientHeight;
      if (textHeight > boxHeight*0.9){
        ...
        longOne = true;
  		...
  		msg.voice = speaker;
  		if (i < 61){
  			msg.addEventListener('end', timeoutA);
  		}
  		window.speechSynthesis.speak(msg);
  	}catch(e){clueRead()}
  }
...
function audioClue(){
	sound.play();
  setTimeout(answerRead, sound.duration*1000+2000);
}
...
function timeoutC(){
  if (isSound){
    audioClue();
  	...
  	timer = setTimeout(answerRead, 4000);
  }
}

function imageInsert(){
  var links = clue2.getElementsByTagName('a');
  if (links.length > 1){
    if (links.length > 2){
    	var maxHeight = heightW/(links.length-1);
      var sizeStyle = "height: " + maxHeight + "px";
      }
    else{
    	imgScaled = window.innerWidth - clue2.clientWidth - 25;
      console.log(imgScaled);
      var gobu = new Image();
      gobu.src = links[1].href;
      if ((imgScaled * gobu.height/gobu.width) > heightW){
      	var maxHeight = heightW
        var sizeStyle = "height: " + maxHeight + "px";
      }
      else{
            sizeStyle = "width: " + imgScaled + "px";
          	maxHeight = 0;
      }
		}
  for (let n = 1; n < links.length; n++){
    var botPos = maxHeight*(n-1);
    var ref = links[n].getAttribute("href");
    if (ref.endsWith(".mp3")){
      sound = document.createElement("audio");
      sound.setAttribute("src", ref);
      sound.setAttribute("style", "position: fixed; left: 0; bottom: 0");
      clue2.appendChild(sound);
      isSound = true;
    }
    else{
      images[n] = document.createElement("img");
      images[n].setAttribute("src", ref);
      images[n].setAttribute("style", "position: fixed; left: 0; bottom: " + botPos + "px; " + sizeStyle);
      clue2.appendChild(images[n]);
      isPic = true;
      }
    }
  }
  if (c==60){
  	if (links.length > 0){
    	if (links.length > 1){
      	maxHeight = heightW/(links.length-1);
        sizeStyle = "height: " + maxHeight + "px";
      }
    	else{
    		imgScaled = window.innerWidth - clue2.clientWidth - 25;
    	  console.log(imgScaled);
  	    var gobu = new Image();
	      gobu.src = links[0].href;
      	if ((imgScaled * gobu.height/gobu.width) > heightW){
    	  	var maxHeight = heightW
  	      var sizeStyle = "height: " + maxHeight + "px";
	      }
      ...
      for (let n = 0; n < links.length; n++){
        ...
        if (ref.endsWith(".mp3")){
          ...
          isSound = true;
        	...
        	images[n+1] = document.createElement("img");
          images[n+1].setAttribute("src", ref);
          images[n+1].setAttribute("style", "position: fixed; left: 0; bottom: " + botPos + "px; " + sizeStyle);
          clue2.appendChild(images[n+1]);
          isPic = true;}
      }
    } 
	}
  ...
  if (isPause){
    ...
    if (isPic){
      for (let n=1; n < images.length; n++){
        clue2.removeChild(images[n]);
      }
      isPic = false;
    }
    if (isSound){
      clue2.removeChild(sound);
      isSound = false;
    }
  	c = 0;
    ...
    try{
      ...
      if (i == 60){
        msg.addEventListener('end', function () {
  			...
  			msg.addEventListener('end', timeoutC);
      }
...
function respondo(aEvent){
  msg.removeEventListener('end', timeoutC);
  msg.removeEventListener('end', timeoutA);
